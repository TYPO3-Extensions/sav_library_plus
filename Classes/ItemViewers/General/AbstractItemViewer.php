...
abstract class AbstractItemViewer {
  ...
  public function getCryptedFullFieldName() {
    return \SAV\SavLibraryPlus\Controller\AbstractController::cryptTag($this->getItemConfiguration('tableName') . '.' . $this->getItemConfiguration('fieldName'));
  }  
	 ...
	 */
 	public function processFuncAttribute($content) {
 		...
 		if (empty($functionName) === FALSE) { 
	      ...
	      \SAV\SavLibraryPlus\Controller\FlashMessages::addError('error.unknownFunction', array($functionName));
	    }
 		}
 	  return $content;		
 	}  
  ...
  protected function arrayToHTML($htmlArray, $noHTMLprefix = FALSE) {
    ...
    if ($this->getItemConfiguration('page' . $special)) {
      $formParameters['folderKey'] =
        \SAV\SavLibraryPlus\Controller\AbstractController::cryptTag($this->getItemConfiguration('page' . $special));
    }
    if ($this->getItemConfiguration('foldertab' . $special)) {
      $formParameters['folderKey'] =
        \SAV\SavLibraryPlus\Controller\AbstractController::cryptTag($this->getItemConfiguration('foldertab' . $special));
    }
    ...
    // Sets the cache hash flag
 		...
 		$additionalParameters = (\SAV\SavLibraryPlus\Managers\UriManager::hasNoCacheParameter() ? array('no_cache' => 1) : array());
    ...
    if ($this->getItemConfiguration('foldertab' . $special)) {
      ...
      $formParameters['subformUidForeign'] =
        $this->getController()->getQuerier()->parseFieldTags($this->getItemConfiguration('subformuidforeign' . $special));	
   	}    
