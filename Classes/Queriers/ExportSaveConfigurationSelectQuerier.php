...
class Tx_SavLibraryPlus_Queriers_ExportSaveConfigurationSelectQuerier extends Tx_SavLibraryPlus_Queriers_ExportSelectQuerier {
  ...
  protected function executeQuery() {
  	...
  	if ($configurationIdentifier > 0) {
  		...
  		$fieldsToUpdate = array_merge($fieldsToUpdate, array('configuration' => serialize($postVariables)));
  		...
  		$fieldsToInsert = array(
  			'pid' => $GLOBALS['TSFE']->id,
  			'cid' => $this->getController()->getExtensionConfigurationManager()->getExtensionContentObject()->data['uid'],
  		...
  		$fieldsToUpdate = array('configuration' => serialize($postVariables));
