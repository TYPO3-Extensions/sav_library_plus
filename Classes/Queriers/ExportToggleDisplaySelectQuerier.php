...
class Tx_SavLibraryPlus_Queriers_ExportToggleDisplaySelectQuerier extends Tx_SavLibraryPlus_Queriers_ExportSelectQuerier {
  ...
  protected function executeQuery() {
    ...
    foreach ($this->rows[0] as $rowKey => $row) {
    	if (empty($postVariables['displaySelectedFields']) === false) {
    		...
    		$this->exportConfiguration['fields'][$rowKey] = (empty($postVariables['fields'][$rowKey]['selected']) ? array('selected' => 0, 'render' => 0) : $postVariables['fields'][$rowKey]);
    	}
    }  	
